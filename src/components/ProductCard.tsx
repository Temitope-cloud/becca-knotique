...
export default function ProductCard({ product }: { product: CatalogProduct }) {
  ...
  return (
    <div className="group @container relative flex min-w-0 flex-col overflow-hidden rounded-2xl border border-stone-200/80 bg-white transition hover:-translate-y-0.5 hover:shadow-[0_24px_60px_-38px_rgba(0,0,0,0.4)]">
      <Link
      ...
      <div className="flex flex-1 flex-col p-3.5">
        ...
        <div className="mt-3.5 flex flex-col gap-2 @min-[250px]:flex-row">
          ...
          >
            ...
            onClick={quickAdd}
            disabled={soldOut}
            className="inline-flex min-h-11 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-stone-900 px-2 py-2.5 text-xs font-semibold text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {added ? (
              <>
                <Check className="h-3.5 w-3.5 shrink-0" /> Added
              </>
            ) : (
              <>
