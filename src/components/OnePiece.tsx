        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 md:flex-row md:items-stretch md:gap-10 lg:gap-12">
          ...
          <div className="relative aspect-3/4 w-full shrink-0 overflow-hidden rounded-xl bg-neutral-200 max-md:mx-auto max-md:max-w-lg md:mx-0 md:aspect-auto md:min-h-[min(28rem,70vh)] md:max-w-none md:flex-1">
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 px-2 pt-10 pb-3 sm:px-3 sm:pb-4">
              <ul
                className="-mx-1 flex touch-pan-x list-none gap-2 overflow-x-auto overflow-y-hidden px-1 pb-1 [scrollbar-width:thin] max-md:snap-x max-md:snap-mandatory sm:justify-center sm:gap-3 md:mx-0 md:flex-wrap md:justify-center md:overflow-x-visible md:px-0"
          ...
          <div className="flex w-full min-w-0 flex-1 flex-col md:max-w-xl md:py-0 lg:max-w-none lg:flex-[1.1]">
            <div className="flex flex-col gap-4 border-b border-gray-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="font-bylast text-3xl sm:text-4xl">One Piece</p>

              <ButtonFill
                href="/"
                btnName="See All"
                icon={<ArrowRight />}
                btnClassName="border-black px-8 py-2 sm:px-10"
            ...
            {onePiece.map((piece, idx) => (
              <div
                key={`${piece.name}-${idx}`}
                className="mt-7 rounded-2xl border border-stone-200/90 bg-linear-to-br from-stone-50/95 via-white to-amber-50/20 p-6 shadow-[0_1px_0_rgba(0,0,0,0.04),0_12px_40px_-12px_rgba(0,0,0,0.08)] ring-1 ring-black/3 sm:p-8"
              >
                ...
                <div className="mt-5 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <p className="font-arial text-4xl font-semibold tracking-tight text-neutral-900">
                    {piece.currentPrice}
                  </p>
                  <p className="text-lg text-stone-400 line-through decoration-stone-300">
                    {piece.oldPrice}
                  </p>
                ...
                <div className="mt-6 flex flex-wrap items-center gap-3 border-b border-stone-200/80 pb-6">
                  ...
                  <p className="text-sm text-stone-500">
                    <span className="font-semibold text-stone-700">12</span>{" "}
                    reviews
                  </p>
                </div>
                ...
                <button
                  type="button"
                  className="mt-6 w-full cursor-pointer rounded-xl bg-neutral-900 px-8 py-3.5 text-base font-semibold tracking-[0.2em] text-white uppercase shadow-lg shadow-neutral-900/25 transition hover:bg-neutral-800 hover:shadow-xl active:scale-[0.99]"
                >
                  {piece.btn}
                </button>
